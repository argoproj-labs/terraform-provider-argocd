...
func expandApplicationSetSpec(d *schema.ResourceData, featureMultipleApplicationSourcesSupported bool, featureApplicationSetApplicationsSyncPolicy bool) (spec application.ApplicationSetSpec, err error) {
	...
	if v, ok := s["sync_policy"].([]interface{}); ok && len(v) > 0 {
		spec.SyncPolicy, err = expandApplicationSetSyncPolicy(v[0].(map[string]interface{}), featureApplicationSetApplicationsSyncPolicy)
		if err != nil {
			return
		}
	}
...
func expandApplicationSetSyncPolicyApplicationsSyncPolicy(p string) (asp application.ApplicationsSyncPolicy, err error) {
	...
	case p == "sync":
		asp = application.ApplicationsSyncPolicyCreateDelete
	default:
		return asp, fmt.Errorf("invalid application sync policy")
	}
	return asp, nil
}

func expandApplicationSetSyncPolicy(sp map[string]interface{}, featureApplicationSetApplicationsSyncPolicy bool) (assp *application.ApplicationSetSyncPolicy, err error) {
	if v, ok := sp["applications_sync"]; ok && featureApplicationSetApplicationsSyncPolicy {
		asp, err := expandApplicationSetSyncPolicyApplicationsSyncPolicy(v.(string))
		if err != nil {
			return nil, err
		}
